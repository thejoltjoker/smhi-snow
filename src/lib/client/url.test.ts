...
describe("SmhiSnowUrl.getMultipointForecast", () => {
  const multipointBase =
    `${BASE_URL}/version/1/geotype/multipoint/time/20260602T180000Z/parameter/air_temperature/data.json`;
  ...
  it("appends a downsample query parameter when provided", () => {
    expect(
      SmhiSnowUrl.getMultipointForecast("20260602T180000Z", "air_temperature", "1", {
        downsample: 10,
      })
...
describe("SmhiSnowUrl.geographicMultipoint", () => {
  const geographicMultipointBase =
    `${BASE_URL}/version/1/geotype/multipoint.json`;
  ...
  it("appends a downsample query parameter when provided", () => {
    expect(
      SmhiSnowUrl.geographicMultipoint("1", { downsample: 10 })
    ).toBe(`${geographicMultipointBase}?downsample=10`);
  });
});
