...
import { SmhiSnowClient } from "../../src/index";
import {
  HIGH_PRECISION_POINT,
  HIGH_PRECISION_POINT_FORMATTED,
} from "../../src/coords";
import { BASE_URL, SmhiSnowUrl } from "../../src/url";

const { longitude, latitude } = HIGH_PRECISION_POINT;

describe("SMHI live API — high-precision point coordinates", () => {
  it("returns 404 when lon/lat are embedded with full floating-point precision", async () => {
    const url = `${BASE_URL}/version/1/geotype/point/lon/${longitude}/lat/${latitude}/data.json`;
    const res = await fetch(url);
    expect(res.status).toBe(404);
  }, 15_000);

  it("resolves a forecast via SmhiSnowClient using the same coordinates (rounded in URL)", async () => {
...
describe("SMHI live API — parameters", () => {
  it("returns parameter definitions with name, unit, and missingValue", async () => {
    const client = new SmhiSnowClient();
...
describe("SMHI live API — geographic area", () => {
  it("returns a GeoJSON polygon for the forecast bounding area", async () => {
    const client = new SmhiSnowClient();
  ...
  it("returns grid point coordinates as GeoJSON MultiPoint", async () => {
    const client = new SmhiSnowClient();
...
describe("SMHI live API — multipoint forecast", () => {
  it("resolves air_temperature grid data with downsample and withGeo query params", async () => {
    const client = new SmhiSnowClient();
  ...
  it("resolves wind_speed grid data for the same time", async () => {
    const client = new SmhiSnowClient();
    const { time } = await client.getTimes();
    const result = await client.getMultipointForecast(
      time[0],
